-- Update update_user_streak function to accept an optional client date for timezone correctness
CREATE OR REPLACE FUNCTION public.update_user_streak(p_user_id uuid, p_client_date date DEFAULT CURRENT_DATE)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Note: workout_date is stored as timestamp without timezone (local time) or just needs to be cast to date
  SELECT EXISTS(
  ...
  IF NOT FOUND THEN
    ...
    VALUES (
      ...
      foods_today_count, p_client_date
    );
    RETURN;
  END IF;
  ...
  ELSIF last_activity = p_client_date - INTERVAL '1 day' THEN
    ...
    WHERE user_id = p_user_id;
  ELSIF last_activity < p_client_date - INTERVAL '1 day' THEN
    -- Reset streak (only if last activity was more than 1 day ago)
    -- Important: We use ELSIF here so we don't accidentally reset if processing an old date for some reason, 
    -- though usually p_client_date should be >= last_activity
    UPDATE user_streaks 
